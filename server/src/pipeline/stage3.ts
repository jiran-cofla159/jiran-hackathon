...
// 사용자가 직무를 수정 확인한 경우에만 프롬프트에 주입(미수정 시 빈 문자열 → 바이트 불변 → 캐시 보존).
const roleNote = (role?: string) =>
  role
    ? `\n\n사용자가 확인한 실제 직무: ${role}. 활동 데이터가 이 직무와 다른 방향으로 치우쳐 있을 수 있으니, 로드맵 우선순위(3a)/질문 선정(3b)은 이 직무 관점을 기준으로 잡아라.`
    : '';

const buildSystem3A = (p: Profile, role?: string) => `${stage3aIntro(p)}
설계 원칙:
...
const buildSystem3B = (p: Profile, role?: string) => `너는 인수인계 문서의 구멍을 찾아 전임자에게 인터뷰 질문을 만드는 역할이다. ${personLine.stage3bIntro(p)}
구멍의 유형:
...
${JSON_ONLY_RULE(QUESTION_TYPE_TEXT)}${roleNote(role)}`;
...
export async function runStage3a(
  ...
  const out = await callLLM<{ items: RoadmapItem[] }>({
    system: buildSystem3A(profile, roleOverride),
...
export async function runStage3b(
  ...
  const out = await callLLM<{ questions: Question[] }>({
    system: buildSystem3B(profile, roleOverride),
